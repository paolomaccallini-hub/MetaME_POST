...
2. If you are on Windows, install WSL (Windows Subsystem for Linux):
```bash
wsl --install
```

3. Download METAL source code from [this page](https://csg.sph.umich.edu/abecasis/Metal/download/) and compile it.

4. Edit `MetaME_config.yml` to set the path to your METAL executable:
```yaml
METAL:
...
6. Submit FUMA jobs using the config files in `FUMA/configs/`. Full dataset identifiers and FUMA job parameters are documented there. FUMA output is available in this repository.

7. Once FUMA results are available, place them in `FUMA/results/` and run the post-processing script:
```r
